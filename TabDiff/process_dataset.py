...
def process_data(name):
    ...
    # ---- pandas>=2/3 compatibility: allow NaN in numeric + string fill in categorical ----
    for df in (train_df, test_df, val_df):
        # numeric columns: allow np.nan (avoid int64 strictness)
        for c in num_columns:
            df[c] = pd.to_numeric(df[c], errors="coerce").astype("float32")

        # categorical columns: allow 'nan' string (avoid int64 strictness)
